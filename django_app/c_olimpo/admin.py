...
class CondominoAdmin(admin.ModelAdmin):
	list_display = ('depto','propietario','cargos','depositos','cuotas','reporte')
	search_fields = ('depto','propietario','poseedor')
	actions = ['genera_reportes']

	def genera_reportes(self, request, queryset):
            for obj in queryset:
               print(" genera reporte %s " % obj.depto)
               envio = "http://192.168.0.60:8090/api-rest/olimpo/informe/%s" % obj.depto
               run(envio)
